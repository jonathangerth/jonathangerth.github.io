document.addEventListener("DOMContentLoaded", function () {
    ...
    function displayCards() {
        ...
        let filteredData = database.filter((item) => {
            return (
                (filters.participant_cat.length === 0 || filters.participant_cat.includes(item.participant_cat.trim())) &&
                (filters.medical_cat.length === 0 || filters.medical_cat.includes(item.medical_cat.trim())) &&
                (filters.outcome_cat.length === 0 || filters.outcome_cat.includes(item.outcome_cat.trim()))
            );
        });
